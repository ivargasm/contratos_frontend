...
export const BloqueClausulasPrestacionServicios = () => {
    ...
    return (
        <div className="space-y-6 text-lg leading-relaxed">
            ...
            <div>
                <p><span className="font-bold">SEGUNDA.- HONORARIOS Y FORMA DE PAGO.</span> Por los servicios objeto de este contrato, &quot;EL CLIENTE&quot; se obliga a pagar a &quot;EL PRESTADOR&quot; la cantidad total de <span className="font-bold">${<InlineInput value={formData.valor_operacion || ''} onChange={handleValorChange} placeholder="0.00" widthClass="w-32" type="number"/>} (<InlineInput value={formData.valor_en_letras || ''} onChange={handleChange('valor_en_letras')} placeholder="Cero pesos 00/100 M.N." widthClass="w-96"/>)</span>, más el Impuesto al Valor Agregado (IVA) correspondiente.</p>
                <p className="mt-2">Dicha cantidad será cubierta de la siguiente manera: <span className="font-bold"><InlineInput value={formData.forma_pago || ''} onChange={handleChange('forma_pago')} placeholder="50% de anticipo y 50% a la entrega final del proyecto." widthClass="w-full"/></span>.</p>
                <p className="mt-2">El pago se realizará en el siguiente lugar y forma: <span className="font-bold"><InlineInput value={formData.lugar_pago || ''} onChange={handleChange('lugar_pago')} placeholder="Cuenta bancaria, cheque o efectivo en las oficinas de 'EL PRESTADOR'." widthClass="w-full"/></span>.</p>
                <p className="mt-2">El pago se efectuará en la fecha estimada: <span className="font-bold"><InlineInput value={formData.fechas_pago || ''} onChange={handleChange('fechas_pago')} placeholder="5 días hábiles" widthClass="w-48"/></span> después de la firma del contrato o según lo acordado entre las partes.</p>
            ...
            <div>
                <p><span className="font-bold">TERCERA.- VIGENCIA.</span> El presente contrato tendrá una vigencia de: <InlineInput value={formData.duracion || ''} onChange={handleChange('duracion')} placeholder="[Duración del contrato]" widthClass="w-48"/> a partir del <span className="font-bold"><InlineInput value={formData.fecha_inicio || ''} onChange={handleChange('fecha_inicio')} placeholder="[Día de Mes de Año]" widthClass="w-48"/></span> y hasta el <span className="font-bold"><InlineInput value={formData.fecha_fin || ''} onChange={handleChange('fecha_fin')} placeholder="[Día de Mes de Año]" widthClass="w-48"/></span>, o hasta la conclusión satisfactoria de los servicios, lo que ocurra primero. Cualquier renovación o extensión deberá constar por escrito.</p>
            </div>
