        ...
        <div className="space-y-6 text-lg leading-relaxed">
            <div>
                ...
                <div className="bg-muted p-4 rounded-md mt-2">
                    <InlineTextArea 
                        value={typeof contratoActual?.form_data?.servicios_descripcion === 'string' ? contratoActual.form_data.servicios_descripcion : ''} 
            ...
            <div>
                <p><span className="font-bold">SEGUNDA.- HONORARIOS Y FORMA DE PAGO.</span> Por los servicios objeto de este contrato, &quot;EL CLIENTE&quot; se obliga a pagar a &quot;EL PRESTADOR&quot; la cantidad total de <span className="font-bold">${<InlineInput value={typeof contratoActual?.form_data?.valor_operacion === 'string' ? contratoActual.form_data.valor_operacion : ''} onChange={handleValorChange} placeholder="0.00" widthClass="" type="number" disabled={contratoActual?.status === 'finalizado'}/>} (<InlineInput value={typeof contratoActual?.form_data?.valor_en_letras === 'string' ? contratoActual.form_data.valor_en_letras : ''} onChange={handleChange('valor_en_letras')} placeholder="Cero pesos 00/100 M.N." widthClass="" disabled={contratoActual?.status === 'finalizado'}/>)</span>, más el Impuesto al Valor Agregado (IVA) correspondiente.</p>
                <p className="mt-2">Dicha cantidad será cubierta de la siguiente manera: <span className="font-bold"><InlineInput value={typeof contratoActual?.form_data?.forma_pago === 'string' ? contratoActual.form_data.forma_pago : ''} onChange={handleChange('forma_pago')} placeholder="50% de anticipo y 50% a la entrega final del proyecto." widthClass="" disabled={contratoActual?.status === 'finalizado'}/></span>.</p>
                <p className="mt-2">El pago se realizará en el siguiente lugar y forma: <span className="font-bold"><InlineInput value={typeof contratoActual?.form_data?.lugar_pago === 'string' ? contratoActual.form_data.lugar_pago : ''} onChange={handleChange('lugar_pago')} placeholder="Cuenta bancaria, cheque o efectivo en las oficinas de 'EL PRESTADOR'." widthClass="" disabled={contratoActual?.status === 'finalizado'}/></span>.</p>
                <p className="mt-2">El pago se efectuará en la fecha estimada: <span className="font-bold"><InlineInput value={typeof contratoActual?.form_data?.fechas_pago === 'string' ? contratoActual.form_data.fechas_pago : ''} onChange={handleChange('fechas_pago')} placeholder="5 días hábiles" widthClass="" disabled={contratoActual?.status === 'finalizado'}/></span> después de la firma del contrato o según lo acordado entre las partes.</p>
            ...
            <div>
                <p><span className="font-bold">TERCERA.- VIGENCIA.</span> El presente contrato tendrá una vigencia de: <InlineInput value={typeof contratoActual?.form_data?.duracion === 'string' ? contratoActual.form_data.duracion : ''} onChange={handleChange('duracion')} placeholder="[Duración del contrato]" widthClass="" disabled={contratoActual?.status === 'finalizado'}/> a partir del <span className="font-bold"><InlineInput value={typeof contratoActual?.form_data?.fecha_inicio === 'string' ? contratoActual.form_data.fecha_inicio : ''} onChange={handleChange('fecha_inicio')} placeholder="[Día de Mes de Año]" widthClass="" disabled={contratoActual?.status === 'finalizado'}/></span> y hasta el <span className="font-bold"><InlineInput value={typeof contratoActual?.form_data?.fecha_fin === 'string' ? contratoActual.form_data.fecha_fin : ''} onChange={handleChange('fecha_fin')} placeholder="[Día de Mes de Año]" widthClass="" disabled={contratoActual?.status === 'finalizado'}/></span>, o hasta la conclusión satisfactoria de los servicios, lo que ocurra primero. Cualquier renovación o extensión deberá constar por escrito.</p>
            </div>
            ...
            <div>
                ...
                <InlineTextArea value={typeof contratoActual?.form_data?.obligaciones_interesado_adicionales === 'string' ? contratoActual.form_data.obligaciones_interesado_adicionales : ''} onChange={handleTextAreaChange('obligaciones_interesado_adicionales')} placeholder="Agregar obligaciones adicionales del cliente (una por línea)" rows={3}/>
            </div>
